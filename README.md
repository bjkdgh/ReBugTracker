...
- 🔐 **灵活的角色权限管理** - 内置管理员、负责人、组内成员、实施组、产品经理五种角色
...
- **[状态工作流程](docs/bug_status_workflow_guide.md)** - 问题状态流转和驳回功能详解
...
| 负责人 | 问题分配、团队管理、状态监控、问题驳回 |
| 组内成员 | 问题处理、状态更新、解决方案提交 |
| 实施组 | 问题提交、进度跟踪、状态查询 |
| 产品经理 | 查看相关团队问题、访问统计数据 |

## 🔄 问题状态流程

系统支持完整的问题生命周期管理，包含以下状态：

| 状态 | 说明 | 可执行操作 | 权限要求 |
|------|------|----------|----------|
| **待处理** | 新提交的问题，等待分配 | 分配给处理人员、驳回 | 负责人 |
| **已分配** | 已分配给具体处理人员 | 开始处理、驳回 | 负责人、组内成员 |
| **处理中** | 正在处理中 | 标记为已解决 | 组内成员 |
| **已解决** | 问题已解决，等待确认 | 确认闭环 | 实施组 |
| **已驳回** | 问题被驳回，需要重新评估 | 重新提交、修改 | 实施组 |
| **已完成** | 问题已确认完成，流程结束 | 查看历史记录 | 所有角色 |
...
**驳回功能说明**：
- **触发条件**：负责人可以驳回"待处理"或"已分配"状态的问题
- **驳回原因**：问题描述不清、重复提交、不属于系统问题等
- **后续处理**：实施组收到驳回通知后，可以修改问题描述重新提交
- **通知机制**：驳回时会自动通知问题提交者和相关人员
